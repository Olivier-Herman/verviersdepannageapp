...
export interface GardeConfig {
  ...
  weekly:        string[]      // rotation hebdo (user_ids)
  wednesday:     string[]      // rotation premier départ du mercredi (user_ids)
...
export function computeGardePlan(cfg: GardeConfig, from: Date, to: Date): GardeDay[] {
  ...
  while (cur <= end) {
    ...
    const weekly = cfg.weekly?.length ? cfg.weekly[mod(weekIndex, cfg.weekly.length)] : null
    const isWed = cur.getDay() === 3
    const nightFirst = isWed
      ? (cfg.wednesday?.length ? cfg.wednesday[mod(weekIndex, cfg.wednesday.length)] : null)
      : (cfg.night_fixed || null)
    out.push({ date: iso(cur), week_no: isoWeek(cur), weekly_garde: weekly, night_first: nightFirst })
