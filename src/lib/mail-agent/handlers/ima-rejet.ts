...
export async function refreshImaSenders(): Promise<void> { IMA_SENDERS = (await getBusinessList('mail_ima_rejets')).map(x => x.toLowerCase()) }
refreshImaSenders().catch(e => console.error('[mail-agent] mail_ima_rejets :', e?.message))
