...
export const BUSINESS_SETTINGS: BusinessSettingDef[] = [
  ...
  { key: 'mail_agent_triage',        label: 'Agent mail — triage quotidien (on / off)',                 group: 'Facturation', kind: 'text',   seed: 'on', help: 'Chaque mail d’info@ et administration@ qui n’est ni un rejet, ni une facture fournisseur, ni du bruit devient une carte « À décider » dans Agent Mail. Olivier 23/09/2026.' },
  ...
  // ── Dispatch ──────────────────────────────────────────────────────────────
  { key: 'rappel_fiche_ouverte_heures',        label: 'Rappel fiche ouverte — 1er rappel (heures après l’attribution)', group: 'Dispatch', kind: 'number', seed: 12, help: 'Une fiche assignée, non clôturée et pas au parc depuis plus de ce nombre d’heures → notification au chauffeur. Transports / rapatriements exclus (plusieurs jours). Olivier 22/09/2026 : 12 h.' },
