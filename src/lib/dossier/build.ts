...
async function buildDossierUncached(anyMissionId: string, light: boolean, price = false): Promise<Dossier | null> {
  ...
  for (const m of legRows) {
    ...
    if (kind === 'gard') {
      ...
      facts.push({ label: 'Sortie', value: exit ? `${fmtD(m.parc_exit_at)} — ${({ relivraison: 'relivraison', sortie: 'sortie du parc', annulation: 'annulation', reparc: 'nouveau séjour', correction: 'correction', facturation: 'facturé, période suivante ouverte', levee_saisie: 'fin de saisie (levée)', abandon: 'abandon volontaire du véhicule', destruction: 'destruction', restitution: 'restitution au client', enlevement_transporteur: 'enlèvement par un transporteur' } as any)[m.parc_exit_reason] || m.parc_exit_reason || ''}` : 'toujours au parc' })
