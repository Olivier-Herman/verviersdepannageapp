...
function ExpandedRow({ missionId, drivers, sources }: { missionId: string; drivers: Driver[]; sources: CatalogSource[] }) {
  ...
  // La vue du dossier (en-tête, fil A/B/C, groupes résumés, fiche complète
  // repliable dans le groupe courant), en mode compact.
  return (
    <div className="border-t bg-page" onClick={e => e.stopPropagation()} onDoubleClick={e => e.stopPropagation()}>
      <DossierGroupsEmbed
        initial={data.dossier}
        fiches={{ [missionId]: data.fiche }}
        shared={{ drivers, sources, userName: data.user.name, userEmail: data.user.email || undefined, userId: data.user.id || undefined, userRole: data.user.role, userModules: data.user.modules || [], userHasOdooAccess: data.userHasOdooAccess, googleMapsKey: data.googleMapsKey }}
        isSuperadmin={false}
...
function AssignAction({ mission, drivers, driverStatuses, onRefresh, onModalChange, userRole, userModules }: {
  ...
  const refuseMission = async () => {
    ...
    const reason = window.prompt('Refuser cette mission ? Indique un motif (optionnel) :', '')
    ...
    try {
      const res = await fetch('/api/missions/confirm', {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ mission_id: mission.id, action: 'refuse', reason: reason.trim() || undefined }),
      })
      const data = await res.json().catch(() => ({}))
      if (!res.ok) {
        alert(`Echec refus : ${data?.error || `HTTP ${res.status}`}`)
        return
      }
      onRefresh()
    } catch (e: any) {
      console.error('[DispatchClient] refuse reseau:', e?.message)
      alert(`Erreur reseau : ${e?.message || 'connexion impossible'}`)
    } finally {
