...
export default async function ChantiersPage() {
  ...
  const [{ data: chantiers, error: dbErr }, { data: logs }, metrics] = await Promise.all([
  ...
  return (
    <AppShell title="Chantiers" userName={u.name || ''} userEmail={u.email || undefined} userId={u.id} userRole={u.role || ''} userModules={u.modules || []}>
      <ChantiersClient initial={(chantiers || []) as Chantier[]} initialLogs={(logs || []) as ChantierLog[]} initialMetrics={metrics}
