...
interface Vehicle {
  ...
  officer_partner_id?: number | null // contact Odoo lié (sélecteur)
...
export default function FourriereClient({ userRole, userName, userEmail, userModules, depotName, depotZoneKeys, parcsNav }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return res
  }, [scopedVehicles, filter, zoneFilter, onlyToPlace, officerFilter, officerPartnerId])
      ...
      <div className="p-4 lg:p-6 space-y-4 ambient-fade-up">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/fourriere/destruction"
              className="flex items-center gap-2 px-3 py-2 bg-surface-2 hover:bg-surface-hover border rounded-xl text-ink-secondary hover:text-ink text-sm transition">
              <Trash2 size={14} />
              Sortie AVP
            </Link>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <div className="relative sm:w-72">
            ...
            <OfficerAutocomplete compact allZones label="Policier" value={officerFilter} companyId={null}
              onChange={setOfficerFilter} onPickPartner={setOfficerPartnerId} />
            {officerFilter && (
              <button type="button" onClick={() => { setOfficerFilter(''); setOfficerPartnerId(null) }} aria-label="Effacer le filtre policier"
