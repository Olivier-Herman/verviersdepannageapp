...
export default function CongesManagerClient({ userRole, userName, userEmail, userModules }: {
  ...
  const [enc, setEnc] = useState<any>({ personnel_id: '', type: 'conge', start_date: '', end_date: '', reason: '', pin: '' })

  const load = useCallback(async () => {
    setLd(true)
    const r = await fetch('/api/conges', { cache: 'no-store' })
    const j = await r.json(); setReqs(j.requests || []); setWorkers(j.workers || []); setLd(false)
  }, [])
  useEffect(() => { load() }, [load])
  ...
  const submitEncode = async () => {
    if (!enc.personnel_id) { alert('Choisis un travailleur.'); return }
    if (!enc.start_date || !enc.end_date) { alert('Indique les dates.'); return }
    if (!enc.pin) { alert('Ton code PIN est requis.'); return }
    setBusy(true)
    try {
      const r = await fetch('/api/conges', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'create_for', ...enc }) })
      const j = await r.json()
      if (j.error) { alert(j.error); return }
      if (j.applied?.missing?.length) alert(`Congé encodé. Mois sans feuille de présence (posés au prochain import) : ${j.applied.missing.join(', ')}.`)
      setEncode(false); setEnc({ personnel_id: '', type: 'conge', start_date: '', end_date: '', reason: '', pin: '' }); await load()
    } finally { setBusy(false) }
  }
    ...
    <AppShell title="Congés" userRole={userRole} userName={userName} userEmail={userEmail} userModules={userModules}>
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <button onClick={() => setEncode(true)} className="ml-auto inline-flex items-center gap-1.5 text-sm px-3 py-2 rounded-lg bg-brand text-white hover:opacity-90"><Plus size={15} /> Encoder un congé</button>
        ...
        <div className="fixed inset-0 z-[200] bg-black/50 flex items-center justify-center p-4">
          <div className="bg-surface border rounded-2xl p-5 w-full max-w-md" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-ink flex items-center gap-2"><CalendarDays size={18} className="text-brand" /> Encoder un congé</h3>
              <button onClick={() => setEncode(false)} className="text-ink-muted hover:text-ink"><X size={18} /></button>
            </div>
            <div className="space-y-3">
              <label className="block"><span className="text-ink-muted text-xs">Travailleur</span>
                <select value={enc.personnel_id} onChange={e => setEnc({ ...enc, personnel_id: e.target.value })} className="w-full mt-1 bg-bg border rounded-lg px-3 py-2 text-sm text-ink">
                  <option value="">— choisir —</option>
                  {workers.map(w => <option key={w.id} value={w.id}>{w.name}{w.kind === 'independant' ? ' (indép.)' : ''}</option>)}
                </select></label>
              <label className="block"><span className="text-ink-muted text-xs">Type</span>
                <select value={enc.type} onChange={e => setEnc({ ...enc, type: e.target.value })} className="w-full mt-1 bg-bg border rounded-lg px-3 py-2 text-sm text-ink">
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block"><span className="text-ink-muted text-xs">Du</span>
                  <input type="date" value={enc.start_date} onChange={e => setEnc({ ...enc, start_date: e.target.value })} className="w-full mt-1 bg-bg border rounded-lg px-3 py-2 text-sm text-ink" /></label>
                <label className="block"><span className="text-ink-muted text-xs">Au</span>
                  <input type="date" value={enc.end_date} onChange={e => setEnc({ ...enc, end_date: e.target.value })} className="w-full mt-1 bg-bg border rounded-lg px-3 py-2 text-sm text-ink" /></label>
              </div>
