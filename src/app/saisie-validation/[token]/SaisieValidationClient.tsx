...
// Page destinataire (Parquet / Frais de justice) : voir l'état de frais + déposer
// la validation (cachet/signature). Publique, sobre. Styles inline (pas d'app shell).

import { useEffect, useRef, useState } from 'react'

interface Info {
  numero: string | null; plate: string | null; vehicle: string | null
  pv: string | null; received: boolean
}

export default function SaisieValidationClient({ token }: { token: string }) {
  ...
  useEffect(() => {
    fetch(`/api/saisie-validation/${token}`, { cache: 'no-store' })
  ...
  const submit = async () => {
    if (!files.length) { setErr('Joignez la validation (PDF ou photo).'); return }
    setBusy(true); setErr(null)
    try {
      const fd = new FormData()
      files.forEach(f => fd.append('files', f))
      if (note.trim()) fd.append('note', note.trim())
      const r = await fetch(`/api/saisie-validation/${token}`, { method: 'POST', body: fd })
        ...
        <div style={S.body}>
          ...
          {state === 'invalid' && (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={S.muted}>Ce lien n'est plus valable. Contactez le service fourrière.</p>
            </div>
          )}
          ...
          {state === 'done' && (
            <div style={{ textAlign: 'center' }}>
              ...
              <h1 style={S.h1}>Validation bien reçue</h1>
              <p style={S.muted}>Merci, votre validation a été rattachée à l'état de frais{info?.numero ? ` ${info.numero}` : ''}.</p>
            </div>
          )}

          {state === 'ready' && info && (
            <>
              <h1 style={S.h1}>Validation de l'état de frais</h1>
              <p style={S.muted}>
                Merci de nous retourner votre validation (cachet / signature) pour l'état de frais ci-dessous.
                {info.numero && <> Réf. <strong>{info.numero}</strong>.</>}
              </p>

              <div style={S.info}>
                <Row k="État de frais" v={info.numero} strong />
                <Row k="Plaque" v={info.plate} />
              ...
              <label style={S.drop} onClick={() => inputRef.current?.click()}>
                ...
                {files.length
                  ...
                  : <span style={{ color: '#64748b' }}>📎 Cliquez pour joindre la validation (PDF ou photo)</span>}
              </label>
              ...
              <button style={{ ...S.btn, opacity: busy ? .6 : 1 }} disabled={busy} onClick={submit}>
                {busy ? 'Envoi…' : 'Envoyer la validation'}
              </button>
