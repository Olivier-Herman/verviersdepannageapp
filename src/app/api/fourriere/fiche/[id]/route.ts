...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const { data: m, error } = await sb
    ...
    .select(`
      ...
      police_blocked, police_levee_saisie_ok,
      ...
      parked_at, storage_waived, loaded_at, received_at, intervention_date, completed_at,
      ...
      temp_garage_out_at, temp_returned_at,
      domaine_remise_date, domaine_vente_date, domaine_at, domaine_note,
...
async function computeProvisionalTariff(_sb: any, m: any): Promise<{ amount_tvac: number | null; htva: number | null; details: string[] }> {
  ...
  try {
    const est = await estimateMissionPrice({
      ...
      storage_waived:    (m as any).storage_waived ?? false,
      delivering_at:     null,
