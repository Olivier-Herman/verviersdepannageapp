...
export async function GET(req: Request) {
  ...
  // Personnel lié à un compte app (= chauffeurs identifiables).
  const { data: personnel } = await sb.from('personnel').select('id, name, user_id, company_code').not('user_id', 'is', null)
  ...
  // Coût : fiches de paie de la période (coût employeur, sinon brut × facteur).
