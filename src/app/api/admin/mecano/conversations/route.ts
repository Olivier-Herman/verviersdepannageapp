...
export async function GET(req: Request) {
  ...
  const { data: rows } = await sb.from('mecano_messages')
    .select('conversation_id, user_name, brand, model, mission_id, role, content, created_at')
    .order('created_at', { ascending: false }).limit(2000)

  const byConv = new Map<string, any>()
  for (const m of (rows || []) as any[]) {
    let c = byConv.get(m.conversation_id)
    if (!c) { c = { conversation_id: m.conversation_id, user_name: m.user_name, brand: m.brand, model: m.model, mission_id: m.mission_id, count: 0, last_at: m.created_at, first_at: m.created_at, last_user_msg: null }; byConv.set(m.conversation_id, c) }
    c.count++
