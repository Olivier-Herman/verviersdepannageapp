...
export async function POST(req: Request) {
  ...
  const rows = items.map((t: any) => ({
    ...
    pricing_mode:          t.pricing_mode === 'brackets' ? 'brackets' : 'forfait',
    ...
    rel_mode:              String(t.mission_type || '').toLowerCase().trim() === 'relivraison' ? (['all_km', 'after_included', 'rem_tariff', 'forfait'].includes(t.rel_mode) ? t.rel_mode : 'forfait') : null,
    ...
    beyond_max_step_price: t.beyond_max_step_price ?? null,
