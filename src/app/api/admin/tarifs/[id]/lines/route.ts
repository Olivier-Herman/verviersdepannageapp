...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const { data: lines, error: lErr } = await sb
    ...
    .select('id, position, kind, name, default_qty, default_price, default_price_majore, apply_surcharges, free_days, parc_count_from, vehicle_class, effective_from, effective_to, notes')
