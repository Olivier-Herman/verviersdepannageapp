...
import {
  ...
  Loader2, ChevronRight, Settings, Trash2
} from 'lucide-react'
...
export default function AdminCheckVehiculeClient() {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation()
    if (!confirm('Supprimer ce contrôle planifié ?')) return
    setDeletingId(id)
    const res = await fetch(`/api/check-vehicule/${id}`, { method: 'DELETE' })
    if (res.ok) {
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {checks.map(c => {
              ...
              const canDelete = ['scheduled', 'pending_claim'].includes(c.status)
