...
import { Plus, Trash2, Loader2, Save, ChevronLeft, Truck, ClipboardList, Users, Pencil, X, Check } from 'lucide-react'
...
export default function CheckVehiculeSettingsClient() {
  ...
  const [editVehicleData, setEditVehicleData] = useState({ name: '', plate: '', usual_driver_id: '' })
  ...
  const startEditVehicle = (v: CheckVehicle) => {
    setEditingVehicle(v.id)
    setEditVehicleData({
      name:             v.name,
      plate:            v.plate,
      usual_driver_id:  v.usual_driver_id || '',
    })
  }

  const cancelEditVehicle = () => {
    setEditingVehicle(null)
    setEditVehicleData({ name: '', plate: '', usual_driver_id: '' })
  }

  const saveEditVehicle = async (id: string) => {
    setSavingEditVehicle(true)
    const res = await fetch(`/api/admin/check-vehicule/settings/vehicles/${id}`, {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        name:            editVehicleData.name,
        plate:           editVehicleData.plate,
        usual_driver_id: editVehicleData.usual_driver_id || null,
      }),
    })
    if (res.ok) {
      const driver = allUsers.find(u => u.id === editVehicleData.usual_driver_id)
      setVehicles(v => v.map(x => x.id === id
        ? { ...x, name: editVehicleData.name, plate: editVehicleData.plate, usual_driver_id: editVehicleData.usual_driver_id || null, driver: driver ? { name: driver.name } : null }
        : x
      ))
      setEditingVehicle(null)
      showMsg('Véhicule modifié')
    }
